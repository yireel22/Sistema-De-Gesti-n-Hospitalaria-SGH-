...
export default function UseCase4() {
    ...
    useEffect(() => {
        fetch("http://localhost:4000/api/caso4")
    ...
    if (loading) return <div className="p-6 text-slate-500 animate-pulse">Modelando perfiles de costos farmacéuticos...</div>;
        ...
        <div className="border border-slate-200 rounded-2xl overflow-hidden">
            <div className="p-6 border-b border-slate-200 bg-slate-50">
                <div className="flex gap-4 items-start">
                    <div className="bg-emerald-500 text-white p-3 rounded-xl"><BadgeDollarSign size={22} /></div>
                    <div>
                        <h2 className="text-2xl font-bold text-slate-800">Caso 4 — Costos y tipos de tratamientos / medicamentos</h2>
                        <p className="text-slate-500 mt-2">Genera asociaciones financieras vinculando el tipo de fármaco recetado, tipo de atención, duraciones y umbrales de coste.</p>
                    </div>
                </div>
            </div>
            <div className="p-6">
                <div className="mb-8">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {data.items.map((item, index) => (
                            <span key={index} className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-medium">{item}</span>
                        ))}
                    </div>
                </div>
                <div className="mb-8">
                ...
                <div className="border border-slate-200 rounded-2xl p-6 bg-slate-50">
                    <h3 className="text-sm font-bold uppercase text-slate-500 mb-4">Reglas de Costos y Gasto</h3>
                    {data.reglas.length === 0 ? <p className="text-slate-500 text-sm italic">No se hallaron reglas asociadas a costos financieros.</p> : (
